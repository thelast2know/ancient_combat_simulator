...
def run_scenario_with_video(scenario_path: str, output_video: str, seed: int = 42) -> bool:
    """
    ...
    if 'blue' in infantry:
        b = infantry['blue']
        world.add_infantry_block(0, b['x_min'], b['y_min'], b['x_max'], b['y_max'])
    if 'red' in infantry:
        r = infantry['red']
        world.add_infantry_block(1, r['x_min'], r['y_min'], r['x_max'], r['y_max'])
    ...
    print(f"Agents: {len(world.agents)}, Duration: {duration} steps ({duration * params.dt:.1f}s sim time)")
    ...
    try:
        for step in range(duration):
            ...
            if step % 5 == 0:
                renderer.save_frame(
                    title=f"Agents: {len([a for a in world.agents if a.alive])}, "
                          f"Collisions: {sum(1 for e in world.events if e.event_type == 'collision')}",
                    debug=True
                )
        ...
        print(f"  Final agents alive: {len([a for a in world.agents if a.alive])}")
...
def test_determinism() -> bool:
    ...
    try:
        ...
        if hashes1 == hashes2:
            ...
            print("✗ Determinism test FAILED: hashes don't match")
            return False
...
def test_collision_safety() -> bool:
    ...
    try:
        ...
        if violations == 0:
            print("✓ Collision safety test PASSED (no overlaps detected)")
            return True
        else:
            print(f"✗ Collision safety test FAILED: {violations} overlap violations")
            return False
...
def test_bounds() -> bool:
    ...
    try:
        ...
        if not (agent.x >= margin - 0.1 and agent.x <= params.arena_width - margin + 0.1):
            violations += 1
            print(f"  X out of bounds: {agent.x}")
        
        if not (agent.y >= margin - 0.1 and agent.y <= params.arena_height - margin + 0.1):
            violations += 1
